...
class ghost {
    ...
    update() { //mainloop of ghost behavior
        ...
        for (let i of inters) { //loop through interestions
            if (i == this.lastUsed) {
                continue
            } //don't use the intersection i just arrived from.
            if (this.pos.dist(i.pos) < gameSpeed) { //am I touching the intersection
                ...
                for (let h = 0; h < 4; h++) { //loop 4 times for each direction
                    ...
                    if (valid) {
                        validDirections.push(h)
                    } //if valid add to valid directions
                }
                ...
                for (let c = 0; c < Object.keys(i.connected).length; c++) { //loop connected intersections
                    ...
                    if (!validDirections.includes(direction)) {
                        continue
                    } //if this isn't a valid direction, skip this intersection
                    if (shortestInter == null) {
                        shortestInter = inter;
                        shortest = direction;
                        continue
                    } //set this to the shortest intersection if there isn't one
                    else {
                        try {
        ...
        if (this.pos.x > textMap[0].length + 1) {
            this.pos.x = -1
        } //wrap the screen
        if (this.pos.x < -1) {
            this.pos.x = textMap[0].length + 1
        } //wrap the screen
    }
